...
struct LocalizedStrings {
    ...
    var installToDevice: String {
        switch language {
        case .japanese: return "デバイスにインストール"
        case .english: return "Install to Device"
        case .chinese: return "安装到设备"
        }
    }
    
    var installDeviceNote: String {
        switch language {
        case .japanese: return "iPhoneをUSBでサーバー（Mac/PC）に接続し、サーバーでideviceinstallerをインストールしてください。"
        case .english: return "Connect your iPhone via USB to the machine running the server. Install ideviceinstaller (libimobiledevice) on the server."
        case .chinese: return "请用USB将iPhone连接到运行服务器的电脑，并在服务器上安装ideviceinstaller（libimobiledevice）。"
        }
    }
    
    var installedSuccessfully: String {
        switch language {
        case .japanese: return "インストールが完了しました"
        case .english: return "Installed successfully"
        case .chinese: return "安装成功"
        }
    }
